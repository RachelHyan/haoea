<template>
  ...
  >
    <div>
      <el-button
        type="primary"
        :link="fileList[0].name !== ''"
      >
        {{ fileList[0].name === "" ? '选择' + fileType : ' 重新选择' }}
      </el-button>
      <el-button
        v-if="fileList[0].name !== ''"
        link
        type="danger"
        @click.stop="remove"
      >
...
export default defineComponent({
  ...
  setup(props,{emit,}) {
    ...
    const getPhoto = (fileUrl: string) => {
      fileList.value[0] = {
        name: fileUrl ? <string>fileUrl.split("/").pop() : "",
        url: fileUrl ? fileUrl : "https://zx-1301451990.cos.ap-guangzhou.myqcloud.com/mes/web/resource/TX-default.png",	
      }
    };
