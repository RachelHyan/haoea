<template>
  <div class="project-management-edit page-root">
    <map-edit
      ref="mapEdit"
    ...
    >
      <template #header>
        <span class="card-header-title">编辑项目</span>
      </template>
      <el-form
        ref="addProjectRef"
...
<script lang="ts">
import { mesProjectProjectInfo, mesProjectQuerySysUserRoleList, mesProjectUpdateProject } from "@/api/mes/projectController";
...
export default defineComponent({
  ...
  setup () {
    ...
    const saveLoading:Ref<boolean> = ref<boolean>(false);
    const saveDialogVisible:Ref<boolean> = ref<boolean>(false);
    ...
    const projectId = ref(parseInt(<string>route.query.id));
    ...
    const getRelationUserInfo = async () => {
		 try {
			 const { data,success, } = await mesProjectProjectInfo({projectId:projectId.value,});
			 if(success) {	
				 addProjectModel.projectName = data.projectName;
				 addProjectModel.firstParty = data.firstParty;
				 addProjectModel.contractAmount = data.contractAmount;
          addProjectModel.projectManagerList = (data.projectManagerList || []).map((item: any) => item.memberId);
				 addProjectModel.projectClerkList = (data.projectClerkList || []).map((item: any) => item.memberId);
				 addProjectModel.projectMapMakerList = (data.projectMapMakerList || []).map((item: any) => item.memberId);
				 addProjectModel.electronicFenceList = data.electronicFenceList;
				 addProjectModel.area = data.area;
				 projectCode.value = data.projectCode;
          mapEdit.value.init(addProjectModel.electronicFenceList);		
			 }
		 }catch(error) {
        console.error(error);
		 }
    ...
    const submit = async() => {
      ...
      try {
        await mesProjectUpdateProject(params);
    ...
    //   projectId.value = parseInt(val as string);
    //   getRelationUserInfo();
    // });
