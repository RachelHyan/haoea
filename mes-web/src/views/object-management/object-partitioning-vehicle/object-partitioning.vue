...
export default defineComponent({
  ...
  setup () {
    ...
    const clickPanelBack = () => {
      ...
      map.setFitView(mapGridOverlay.getOverlays());
			
      mapObjectOverlay.hide();
      infoWindow.close();
    };

    const setGridDefaultStyle = () => {
      tablePanelRef.value?.gridTableRef?.load();
      ...
      const clickClose = () => {
        ...
        if (isGrid) {
          ...
          try {
            ...
            workArray.value = workArray.value = res.geometryType === 1 ? res.routePointList.flat() : res.routePointList;
