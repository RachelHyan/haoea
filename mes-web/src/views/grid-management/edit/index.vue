<template>
  <div class="grid-management-edit page-root">
    <map-edit
      ref="mapEdit"
...
<script lang="ts">
import { mesGridQueryGridInfo, mesGridUpdateGrid } from "@/api/mes/gridController";
...
export default defineComponent({
  ...
  setup () {
    ...
    const gridId = parseInt(<string>route.query.id);
    ...
    const teamList = reactive<MES.TeamDTO[]>([]);
    const bindStatus = ref<boolean>(false);
    ...
    const getGrid = async () => {
      try {
        let {data,} = await mesGridQueryGridInfo({gridId,});
        form.gridName = data.gridName;
        form.teamId = data.teamId;
        form.color = data.color ? data.color : "#000000";
        form.area = data.area;
        bindStatus.value = data.bindStatus;
        gridPoint.value = data.gridPointList;
        changeColor(form.color);
        mapEdit.value.init(data.gridPointList);
      }catch(error) {
        console.error(error);
      }
    };
