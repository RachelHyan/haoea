...
import { mesPersonVerifyFace } from "@/api/mes/personController";
  ...
  setup() {
    ...
    const getPunchData = async () => {
      try {
        ...
        !data && uni.showToast({ title: "未获取到人员信息", content: "请联系管理员", })
    ...
    const handlePunchClock = () => {
      uni.navigateTo({
        url: "/pages/camera/index?watermark=true&face=true",
        events: {
          save: async (res: any) => {
            ...
            try {
              ...
              logger.error("handlePunchClock=>人脸识别", error)
            }
          },
        },
      })
    ...
    const goToRectify = () => {
      uni.reLaunch({ url: "/pages/rectify-list/index", })
