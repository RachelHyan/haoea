<template>
  <view class="inspector-list">
    ...
    >
      ...
      <template #row="{row}: {row: MES.WechatUserInspectionDTO}">
        <view
        ...
        >
          ...
          <view
            ...
            :class="{'project-board-list-item-right--error': false}"
          >
            {{ filterStatusData.find(item => item.value === filterObject.status)?.label }}
          </view>
        </view>
...
<script lang='ts'>
import { mesWechatIndexSelectInspectionUserList } from "@/api/mes/wechatController";
...
export default defineComponent({
  ...
  setup(props){
    ...
    const getList = async () => {
      const params: MES.WechatIndexSelectInspectionUserListParams = {projectId, jobType: props.jobType,}
      filterObject.status !== "all" && (params.inspectionStatus = filterObject.status)
      filterObject.search !== "" && (params.userName = filterObject.search)
      const {data,} = await mesWechatIndexSelectInspectionUserList(params)
      return { data, }
    }
    ...
    const handleItem = (row: MES.WechatUserInspectionDTO) => {
      const params = {
        pageType: "inspector",
